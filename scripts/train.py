...
def train(cfg: DictConfig):
    ...
    mini_batch_size = cfg.algo.mini_batch_size
    ...
    # 1. 实例化环境和网络
    envs = SharedMemoryVectorEnv(num_envs=num_envs, obs_dim=obs_dim, action_dim=action_dim)
    agent = ActorCritic(obs_dim=obs_dim, action_dim=action_dim).to(device)
    optimizer = torch.optim.Adam(agent.parameters(), lr=lr, eps=1e-5)
    
    # 2. 轨迹缓冲区分配
    obs_buffer = torch.zeros((rollout_steps, num_envs, obs_dim)).to(device)
    action_buffer = torch.zeros((rollout_steps, num_envs, action_dim)).to(device)
